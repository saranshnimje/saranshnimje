#!/usr/bin/env python3
"""
Generate stack.svg card for GitHub profile README.
Creates a terminal-style technology stack visualization.
"""
...
def generate_stack_svg(output_path: str = None) -> str:
    """Generate stack.svg based on profile configuration."""
    config = get_config()
    
    if output_path is None:
        output_path = Path(__file__).parent.parent / "assets" / "cards" / "stack.svg"
    ...
    # Build content lines
    lines = [
        f"{config.username}@github:~$ cat stack.json",
        "",
    ]
...
<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">
  <title>Technology stack for {config.name}</title>
  <desc>Terminal-style card showing programming languages, frameworks, and tools</desc>
...
if __name__ == "__main__":
    ...
    result = generate_stack_svg(output_file)
